...
// Smooth knock-down, unconscious twitch and recovery animation. On 1.21.1 the renderer
// still receives the entity itself, so the mixin hands one straight over.
...
public final class StunnedZombieRenderHandler {
    ...
    private static final ThreadLocal<ArrayDeque<Boolean>> PUSHED = ThreadLocal.withInitial(ArrayDeque::new);

    private StunnedZombieRenderHandler() {}

    public static void before(LivingEntity entity, PoseStack pose) {
