...
public final class SoundWaveRenderer {
    ...
    public static void register() { RenderHandEvent.BUS.addListener(SoundWaveRenderer::render); }
    public static void activate(int playerId) { STARTS.put(playerId, System.currentTimeMillis()); }

    private static boolean render(RenderHandEvent event) {
        ...
        for(var entry:STARTS.entrySet()){
            ...
            for(int pulse=0;pulse<3;pulse++){
                ...
                event.getNodeCollector().submitCustomGeometry(pose,RenderTypes.debugQuads(),
