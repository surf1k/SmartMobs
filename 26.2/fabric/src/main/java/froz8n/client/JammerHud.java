...
public final class JammerHud {
    ...
    /** Registered as a HUD element by {@link SmartMobsClient}. */
    public static void render(GuiGraphicsExtractor g,DeltaTracker delta){
        Minecraft mc=Minecraft.getInstance();
        if(mc.player==null||(!mc.player.getMainHandItem().is(SmartMobs.SOUND_JAMMER)
                &&!mc.player.getOffhandItem().is(SmartMobs.SOUND_JAMMER)))return;
        ...
        g.text(mc.font,Component.literal("SM-JAMMER // SYS.R2"),x+7,y+6,0xFF7CFF9D,false);
        g.text(mc.font,Component.literal("SHIFT + MWHEEL : SELECT"),x+7,y+17,0xFF527C5E,false);
    ...
    private static void drawMode(GuiGraphicsExtractor g,Minecraft mc,int x,int y,int width,int thisMode,long ready,long total,int accent,String title){
        ...
        g.text(mc.font,(selected?"> ":"  ")+title,x+8,y+4,selected?accent:0xFF6B8673,false);
        String status=ready<=now?"READY":String.format("%.1fs",(ready-now)/1000F);int sw=mc.font.width(status);
        g.text(mc.font,status,x+width-sw-6,y+4,ready<=now?0xFF67EF8B:0xFFFFCE73,false);
